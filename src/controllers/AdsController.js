...
module.exports = {
  ...
  addAction: async (req, res) => {
    ...
    if (!title || !cat) {
      res.json({ error: 'O titulo e/ou categoria não foram preenchidos!' })
      return
    }
    ...
    if (req.files && req.files.img) {
      if (req.files.img.length == undefined) {
        ...
        for (let i = 0; i < req.files.img.length; i++) {
          if (['image/jpeg', 'image/jpg', 'image/png'].includes(req.files.img[i]) && req.files.img[i].mimetype)  {
  ...
  editAction: async (req, res) => {
    ...
    if (user._id.toString() !== ad.idUser) {
      res.json({ error: 'Este anúncio não é desse propietário!' })
      return
    }
